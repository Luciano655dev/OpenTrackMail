...
| `https://opentrackmail.vercel.app/*` host permission | Calls the production API and scopes the sender-side pixel block to that host. Developer builds also request `http://localhost:3001/*` for the local API. |
...
6. Use `https://opentrackmail.vercel.app/privacy` as the privacy policy URL.
