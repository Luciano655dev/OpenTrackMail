...
begin
  ...
  -- Serialize all requests for one tracked email. This guarantees that two
  -- simultaneous first loads cannot both pass the first-request heuristic.
  ...
  -- Gmail commonly fetches a new remote image as part of sending or proxying.
  -- The first request is retained for diagnostics but deliberately never shown
  -- as a recipient open. This favors avoiding false positives over detecting a
  -- first genuine open from a provider that performs no automatic fetch.
  event_classification := case
    when not has_prior_request then 'early_automatic'
