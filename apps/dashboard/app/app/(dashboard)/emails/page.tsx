...
export default async function EmailsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <div><div className="page-title"><div><h1>Tracked emails</h1><p>Open activity from messages sent with the extension.</p></div><dl className="metrics"><div><dt>Tracked</dt><dd>{total || 0}</dd></div><div><dt>Opened</dt><dd>{opened || 0}</dd></div><div><dt>Open rate</dt><dd>{rate}%</dd></div></dl></div>
    <div className="table-controls"><form className="search-form"><Search size={16}/><input name="search" defaultValue={search} placeholder="Search recipient or subject" aria-label="Search tracked emails"/><input type="hidden" name="status" value={status}/></form><div className="filters" aria-label="Status filter">{[["all","All"],["opened","Opened"],["sent","Not opened"]].map(([value,label])=><Link className={status===value?"active":""} key={value} href={`/app/emails?status=${value}${search?`&search=${encodeURIComponent(search)}`:""}`}>{label}</Link>)}</div></div>
